...
const client = new Client({
  user: 'postgres',
  host: 'db.rwynuhrvgwmykokuxhie.supabase.co',
  database: 'postgres',
  password: 'vibeshop@76281557',
  port: 5432,
  ssl: {
...
async function setup() {
  try {
    ...
    if (prodCount === 0) {
      console.log(`📦 Inserting ${generatedProducts.length} initial products...`);
      for (const p of generatedProducts) {
        await client.query(
          `INSERT INTO products (
            id, name, description, price, original_price, rating, review_count, 
            images, category, tags, colors, sizes, stock, details, reviews
      ...
      console.log(`ℹ️ "products" table already has ${prodCount} items. Skipping insertion.`);
    }

    // Check if coupons has entries
    const couponCheck = await client.query('SELECT COUNT(*) FROM coupons');
    const couponCount = parseInt(couponCheck.rows[0].count, 10);

    if (couponCount === 0) {
      console.log(`🎟️ Inserting ${mockCouponCodes.length} initial coupons...`);
      for (const cp of mockCouponCodes) {
        await client.query(
          `INSERT INTO coupons (code, discount_rate, active) VALUES ($1, $2, $3)`,
          [cp.code, cp.discountRate, cp.active]
        );
      }
      console.log('✅ Coupons inserted.');
    } else {
      console.log(`ℹ️ "coupons" table already has ${couponCount} items. Skipping insertion.`);
    }
